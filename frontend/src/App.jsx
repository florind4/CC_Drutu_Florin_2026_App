    ...
    <div className="telemetry-dashboard">
      <header className="telemetry-header">
        ...
        <p className="telemetry-meta">
          Logged in as: <strong>{role}</strong> {deviceId && <span> | Device Context: <strong>{deviceId}</strong></span>}
        </p>
      </header>
...
function App() {
  ...
  useEffect(() => {
    ...
    // Fetch Data with Robust JSON handling to fix "Unexpected end of JSON input"
    setLoadingData(true);
    setDataError("");
    fetch(`${API_BASE}/api/data`, { headers: { Authorization: `Bearer ${idToken}` } })
      .then(async (res) => {
        ...
        if (!text) return { data: [], role: '', device_id: '' };
  ...
  const handleSignOut = () => {
    auth.signoutRedirect({ extraQueryParams: { client_id: OIDC_CONFIG.client_id, logout_uri: LOGOUT_URI } });
  };
