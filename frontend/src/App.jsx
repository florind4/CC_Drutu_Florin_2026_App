...
// 1. TELEMETRY DASHBOARD COMPONENT
function TelemetryDashboard({ rows, role, deviceId, loading, error }) {
  if (loading) return <p className="muted">Loading live telemetry data...</p>;
  if (error) return <p className="telemetry-error">{error}</p>;
  if (!rows.length) return <p className="telemetry-empty">No telemetry data available.</p>;
...
function App() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/profile`, { headers: { Authorization: `Bearer ${idToken}` } })
      .then(res => res.json()).then(setProfile).finally(() => setLoadingProfile(false));

    setLoadingData(true);
    fetch(`${API_BASE}/api/data`, { headers: { Authorization: `Bearer ${idToken}` } })
      .then(res => res.json()).then(setDataResponse).catch(err => setDataError(err.message)).finally(() => setLoadingData(false));
  }, [idToken]);
        ...
        {!auth.isAuthenticated ? (
          ...
          <div className="grid">
            ...
            <section className="card">
              <h2>Auth Token</h2>
              <button className="btn btn-small" onClick={() => setShowToken(!showToken)}>{showToken ? "Hide" : "Show"}</button>
            ...
            <section className="card card-wide">
              <TelemetryDashboard rows={rows} role={role} deviceId={deviceId} loading={loadingData} error={dataError} />
            </section>
          </div>
        )}
      </main>
